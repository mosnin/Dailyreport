...
import { Check, Flame, PenLine, NotebookPen } from "lucide-react";
...
export default function TodayPage() {
  ...
  const todayStatus = useTodayStatus(convexUserId);
  ...
  const reportDone = todayStatus.reportDone;
  const streak = todayStatus.streak;

  const morningBrief = (extraClass: string, delay: number) => (
    <BentoCard className={extraClass} delay={delay}>
  ...
  return (
    <div className="space-y-4 pb-6">
      <PageHeader
        ...
        subtitle="Start with your daily report, then keep your trackers climbing."
        action={
          hasTrackers && total > 0 ? (
            <Link
              href="/log"
              className="flex items-center gap-2 rounded-full bg-accent px-4 py-2.5 text-sm font-semibold hover:bg-accent/70"
            >
              <PenLine className="h-4 w-4" />
              {doneCount >= total ? "Trackers logged" : "Log trackers"}
            </Link>
          ) : undefined
        }
      />

      {/* Foundation: the daily report + your life score */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
        <BentoCard href="/reports/daily" className="lg:col-span-2 flex flex-col justify-between min-h-[168px]" delay={0.02}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/70">Daily report</span>
              <h2 className="mt-1.5 text-xl font-semibold leading-tight">{reportDone ? "Today's report is in" : "Write today's report"}</h2>
              <p className="mt-1.5 text-sm text-muted-foreground max-w-md leading-snug">
                The reflection at the core of your day: what you did, who you saw, problems to solve and tomorrow&apos;s plan.
              </p>
            </div>
            {streak > 0 && (
          ...
          <span className="mt-5 inline-flex w-fit items-center gap-2 rounded-full bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground">
            <NotebookPen className="h-4 w-4" />
            {reportDone ? "Review today" : "Start reflection"}
          </span>
        </BentoCard>

        {/* Life score */}
        <BentoCard href="/dashboard" className="flex flex-col items-center justify-center text-center" delay={0.05}>
          ...
          <p className="text-xs text-muted-foreground mt-3">{overview?.hasScored ? scoreLabel(overview.composite) : "Add trackers"}</p>
        </BentoCard>
      </div>
      ...
      {/* Build on top: trackers */}
      {hasTrackers ? (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {/* Checklist */}
            <BentoCard className="col-span-2" delay={0.04}>
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-semibold">Today&apos;s trackers</h2>
            ...
            {/* Morning brief */}
            {morningBrief("col-span-2 lg:col-span-1 flex flex-col justify-between min-h-[140px]", 0.08)}
          </div>
        ...
        <>
          {morningBrief("flex flex-col gap-2 min-h-[120px]", 0.07)}
          {/* Build on top */}
          <BentoCard delay={0.1}>
            <h2 className="font-semibold mb-1">Measure more on top</h2>
            <p className="text-sm text-muted-foreground mb-3">Your daily report is the foundation. Add trackers to score anything else over time, fitness, money, a habit. Describe it and the AI builds the log and scoring.</p>
